...
/// Queued callbacks the final drain of a finished task may still deliver.
...
pub enum Request {
    ...
    /// Fire `SessionEnd` for one session, then reap the jobs it owned.
    ...
    CallTool {
        ...
        live: Option<LiveCtx>,
        /// The caller already holds the in-flight slot, so this call rides it
        /// instead of taking one of its own (see [`under_inflight_slot`]).
        nested: bool,
    },
    ComputeHeader {
        plugin: Arc<str>,
    ...
    fn sandboxed(plugin_dir: &Path) -> Option<Self> {
        ...
        if !dir.starts_with(&root) {
            tracing::warn!(
                ...
                "plugin lua dir escapes the plugin; require() searches nothing"
            );
            return None;
        }
        Some(Self {
...
/// Runs `fut` on the Lua thread on behalf of a call that is waiting on it:
///
...
async fn run_scoped<F: Future>(lua: &Lua, scope: TaskScope, fut: F) -> F::Output {
    ...
    let pump = async {
        ...
        loop {
            ...
            drain_job_events(lua, &owner, &mut event_buf);
    ...
    // `or` drops the pump the moment `fut` wins, so events that landed in
    // between would go with the scope. One bounded pass delivers them: a job
    // printing faster than we deliver cannot park shutdown here.
    deliver_pending_job_events(lua, &handle, FINAL_DRAIN_BUDGET).await;
    drop(scope);
    out
}

/// Fire up to `budget` queued job events, one at a time so a dropped caller
/// cannot strand a batch. Yields between callbacks: one with nothing to await
/// never does, so a job printing faster than we deliver would hold the
/// executor here forever.
async fn deliver_pending_job_events(lua: &Lua, handle: &TaskHandle, budget: usize) {
    for _ in 0..budget {
        let owner = JobOwner::Task(lock_cell(handle).id);
        let Some((job_id, event)) = with_jobs(lua, |store| store.next_event(&owner)) else {
            return;
        };
        if let Err(e) = deliver_job_event(lua, job_id, &event).await {
            tracing::warn!(error = %strip_traceback(&e), "detached job callback failed");
...
/// Batch `owner`'s pending job events into `buf`. The store hands them out one
/// at a time, but delivering a callback re-enters [`with_jobs`], so they have
/// to be collected before the borrow is released rather than as they are used.
fn drain_job_events(lua: &Lua, owner: &JobOwner, buf: &mut Vec<(u32, JobEvent)>) {
    with_jobs(lua, |store| {
        while let Some(item) = store.next_event(owner) {
            buf.push(item);
        }
    });
}

/// The plugin-owned counterpart of [`drain_job_events`].
fn drain_plugin_job_events(lua: &Lua, buf: &mut Vec<(u32, JobEvent)>) {
    with_jobs(lua, |store| {
        while let Some(item) = store.next_plugin_event() {
            buf.push(item);
        }
    });
}
...
struct PluginOwner {
    ...
    /// Shared lock on the revision directory this owner runs from, if any.
    /// Held until the owner is dropped so a prune cannot delete code that is
    /// still running.
...
impl LuaRuntime {
    ...
    async fn load_source(
        ...
        // After `replace_plugin` above, so a rule may name one of this very
        // load's tools; the registry it is weighed against is the committed one.
        let rules = crate::api::tool::resolve_rules(&self.registry, &name, &permissions, rules);
        ...
        .spawn(move || {
            ...
            let ex = Rc::new(smol::LocalExecutor::new());
            {
                ...
                ex.spawn(async move {
                    let mut event_buf = Vec::new();
                    loop {
                        drain_plugin_job_events(&lua, &mut event_buf);
                        if !event_buf.is_empty() {
                    ...
                    match msg {
                        ...
                        Request::CallTool {
                            ...
                            ex.spawn(async move {
                                ...
                                let res = covered(slot, run_tool_call(
                                    lua.clone(),
                                    plugin,
                                    tool,
                                    input,
                            ...
                            let res = match found {
                                Some(bp) => {
                                    ...
                                    if let Some(src) = init {
                                        rt.load_source(
                                            Arc::clone(&name),
                                            PluginLoad::Chunks(&[LoadChunk::new(
                                                name.as_ref(),
                                                src,
                                            )]),
                                            LoadContext::plain(
                        ...
                        Request::EndSession(end) => {
                            // Measured at dispatch, not when the request was
                            // queued, so a handler is told what is left rather
                            // than what was promised.
                            let deadline_ms = end.wait.as_ref().map(|(at, _)| {
                            ...
                            crate::api::autocmd::dispatch(
                                rt.lua.clone(),
                                SESSION_END_EVENT.to_owned(),
                                None,
                                data,
                            )
                            .await;
                            with_jobs(&rt.lua, |store| {
                                store.kill_session(&rt.lua, end.session)
                            });
...
mod tests {
    ...
    /// A call still queued for a slot has no task cell, so nothing else can end
    /// its wait once the caller stops waiting for the reply.
    ...
    fn until_abandoned_ends_a_parked_handler_only_after_its_window() {
        ...
        smol::block_on(async {
            ...
            let _ = until_abandoned(
                std::future::ready(Ok::<LuaValue, mlua::Error>(LuaValue::Boolean(true))),
