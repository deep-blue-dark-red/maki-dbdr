...
impl Cadence {
    ...
    /// Slows clock-driven repaints to at most one per `floor`, keeping the
    /// motion flag so whatever is moving still gets its frames, just fewer of
    /// them. For when the pixels are real but nobody is looking at them: a
    /// backgrounded terminal still streams, it just does not need 60 FPS of
    /// spinner. Real events bypass this entirely — they wake the loop.
    pub fn throttled(self, floor: Duration) -> Self {
        Self {
            frame: self.frame.map(|f| f.max(floor)),
            moves: self.moves,
        }
    }
